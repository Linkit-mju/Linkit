...
After rebasing the deployment-only change onto `origin/main`, the frontend lint
and build, backend tests, and combined Docker build passed again. The main-base
frontend has no `test` script, so no frontend unit-test command was available.
