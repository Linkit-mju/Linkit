...
1. 플랫폼 관리자가 실제 활동 중인 동아리·학생회·자치기구를 확인한다.
2. 관리자가 조직과 영문·숫자 6자리 초대코드를 seed data로 등록한다.
3. 학생에게는 조직 생성 또는 개설 신청 기능을 제공하지 않는다.

### 5.2 구성원 가입

1. 사용자가 관리자로부터 전달받은 영문·숫자 6자리 초대코드를 입력한다.
2. 유효한 코드이면 해당 조직의 구성원으로 가입한다.
3. 유효하지 않은 코드와 이미 가입한 조직은 각각 명확한 오류와 다음 행동을 안내한다.
4. 가입된 사용자에게 역할에 맞는 권한이 부여된다.
...
- 영문·숫자 6자리 초대코드를 통한 기존 조직 가입
- 계정 비활성화 및 탈퇴

#### 조직 및 권한

- 관리자가 seed data로 등록한 조직과 초대코드 관리
- 임기, 부서, 직책 관리
...
| POST | `/auth/email-verifications/confirm` | 이메일 인증 토큰 확인 |
| POST | `/auth/email-verifications/resend` | 인증 메일 재전송 |
| POST | `/auth/login` | 로그인 |
| GET | `/me` | 내 정보와 소속 조회 |
| GET | `/organizations/{organizationId}` | 조직 조회 |
| PATCH | `/organizations/{organizationId}` | 조직 정보 수정 |
| POST | `/organizations/join` | 6자리 초대코드로 조직 가입 |
...
- 가입 직후 계정은 이메일 인증 대기 상태이며, 30분 유효한 일회용 링크를 확인하기 전에는 세션 로그인을 허용하지 않는다.
- 인증 토큰 원문은 메일 링크에만 포함하고 서버에는 SHA-256 해시만 저장한다. 재전송 시 기존 토큰을 폐기한다.
- 학교 이메일 도메인만으로 학생회 소속을 증명할 수 없으므로 관리자가 사전 등록한 조직의 초대코드로 가입을 제한한다.
...
| 리스크 | 대응 |
|---|---|
| 학생회 소속 검증의 어려움 | 학교 이메일 인증과 사전 등록된 조직의 초대코드 결합 |
| 전임자가 후임자 계정을 미리 알 수 없음 | 조직 초대코드 가입 후 관리자의 담당자 재지정 |
...
8. 운영 주체와 조직 seed data 및 초대코드 관리 담당자
...
- 학교 공식 인증 연동 전까지 `@mju.ac.kr` 이메일 인증과 사전 등록된 조직의 초대코드를 사용한다.
